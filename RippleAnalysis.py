...
class RippleDetector(ThreadExtension.StoppableProcess):
    ...
    def __init__(self, lfp_listener, calib_plot, \
        ...
        self._var_ripple_power = np.reshape(np.frombuffer(self._shared_var_ripple_power, dtype='double'), (self._n_tetrodes))
        ...
        np.copyto(self._var_ripple_power, self._std_ripple_power * self._std_ripple_power)
        ...
        while not self.req_stop():
            ...
            if self._lfp_consumer.poll():
                ...
                if (lfp_window_ptr == RiD.LFP_FILTER_ORDER):
                    ...
                    baseline_ripple_power = current_ripple_power[self._ripple_baseline_tetrode.value]
                    ...
                    if (self._update_ripple_stats.value) and (self._n_data_pts_seen.value < RiD.STAT_ADJUSTMENT_DATA_PTS):
                        self._n_data_pts_seen.value += 1
